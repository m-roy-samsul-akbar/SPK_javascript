...
public class Main extends javax.swing.JFrame {
    ...
    private void btnUpdateAlternatifActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnUpdateAlternatifActionPerformed
        ...
        String ipk = tabelAlternatif.getValueAt(i, 2).toString();
        String pengalaman_kerja  = tabelAlternatif.getValueAt(i, 3).toString();
        String jumlah_bahasa = tabelAlternatif.getValueAt(i, 4).toString();
        String jumlah_sertifikat = tabelAlternatif.getValueAt(i, 5).toString();
        String jumlah_project = tabelAlternatif.getValueAt(i, 6).toString();
        int id_int = Integer.parseInt(id);
//        String id = tabelKriteria.getValueAt(i, 0).toString();
//        String kriteria = tabelKriteria.getValueAt(i, 1).toString();
//        String bobot = tabelKriteria.getValueAt(i, 2).toString();
...
//        int id_int = Integer.parseInt(id);
        ...
        A.setIpk(ipk);
        A.setPengalaman_kerja(pengalaman_kerja);
        A.setJumlah_bahasa(jumlah_bahasa);
        A.setJumlah_sertifikat(jumlah_sertifikat);
        A.setJumlah_project_terselesaikan(jumlah_project);
//        p.setId(id_int);
//        p.setKriteria(kriteria);
//        p.setBobot(bobot);
//        p.setLabel(label);
    ...
    public static void loadDataAlternatif() {
        try {
            // table design
            Object[][] data = null;
            Object[] header = {"ID", "ALTERNATIF", "IPK", "PENGALAMAN KERJA", "JUMLAH BAHASA", "JUMLAH SERTIFIKAT", "JUMLAH PROJECT"};
            ...
            while (rs.next()) {
                ...
                double pengalaman_kerja = rs.getDouble("pengalaman_kerja");
                double jml_bahasa = rs.getDouble("jml_bahasa");
                double jml_sertifikat = rs.getDouble("jml_sertifikat");
                double jml_project = rs.getDouble("jml_project");
                
                Object[] rowData = {id, alternatif, ipk, pengalaman_kerja, jml_bahasa, 
                    jml_sertifikat,jml_project};
    ...
    private void SPK_SAW() {
        try {
            ...
            while (rs.next()) {
                ...
                double norm_ipk = label("ipk").equals("ipk") ? MIN("ipk") / ipk : ipk / MAX("ipk");
                double norm_pengalaman_kerja = label("pengalaman_kerja").equals("cost") ? MIN("pengalaman_kerja") / pengalaman_kerja : pengalaman_kerja / MAX("pengalaman_kerja");
                double norm_jml_bahasa = label("jml_bahasa").equals("cost") ? MIN("jml_bahasa") / jml_bahasa : jml_bahasa / MAX("jml_bahasa");
                double norm_jml_sertifikat = label("jml_sertifikat").equals("cost") ? MIN("jml_sertifikat") / jml_sertifikat : jml_sertifikat / MAX("jml_sertifikat");
                double norm_jml_project = label("jml_project").equals("cost") ? MIN("jml_project") / jml_project : jml_project / MAX("jml_project");
//                double norm_keawetan = label("keawetan").equals("cost") ? MIN("keawetan") / keawetan : keawetan / MAX("keawetan");

                
                double bobot_akhir = (bobot("ipk")*norm_ipk) + (bobot("pengalaman_kerja")*norm_pengalaman_kerja) + (bobot("jml_bahasa")*norm_jml_bahasa) + (bobot("jml_sertifikat")*norm_jml_sertifikat) + (bobot("jml_project")*norm_jml_project);
                nomor++;
